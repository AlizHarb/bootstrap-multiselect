...
class AzhSelect {
    ...
    _updateDisplay() {
        ...
        if (this.isMultiple) {
            this.display.innerHTML = selectedOpts.map(o => this.options.templateValue(o)).join(" ");
        } else {
        ...
        if (this.searchInput) {
            this.searchInput.addEventListener("input", (e) => {
                ...
                if (this.options.remote && val.length >= this.options.minQuery) {
                    ...
                    this._remoteTimer = setTimeout(() => this._fetchRemote(val), this.options.delay);
                    return;
                }

                this.filteredOptions = this.optionsData.filter(o => o.label.toLowerCase().includes(val));
    ...
    _hasOption(val) {
        return this.optionsData.some(o => o.value.toLowerCase() === val.toLowerCase());
    }
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                data.forEach(item => {
                    ...
                    const label = item[labelField] ?? item.name ?? item.title ?? String(value);
                ...
                if (query) {
                    ...
                    this.filteredOptions = this.optionsData.filter(o => o.label.toLowerCase().includes(q));
                } else {
                    this.filteredOptions = [...this.optionsData];
                }
    ...
    _trigger(name, detail = {}) {
        const event = new CustomEvent(`azh:${name}`, { detail: { instance: this, ...detail } });
